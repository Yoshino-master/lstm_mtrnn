...
#     def __init__(self, io_size, fast_hidden_size, slow_hidden_size, cls_nums, tau_io=2.0, tau_fh=5.0, tau_sh=70.0, tau_cls=10.0):
    def __init__(self, n_in=28, n_hidden=100, cls_nums=10, tau=3.0):
    ...
    def forward(self, data):
        ...
        c = torch.zeros(data.shape[1], self.n_hidden).cuda()
        for i in range(data.shape[0]):
            h, c = self.lstm(data[i], h, c)
        out = self.fc(h)
        return out
...
#         h = (1-1/self.tau) * h + self.tau * (o * torch.tanh(c))
        h = o * torch.tanh(c)
        return h, c
